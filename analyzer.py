...
def analyze_from_bytes(image_bytes: bytes) -> tuple[list[dict], str]:
    """Analiza desde bytes — compatible con Windows, Linux y Mac."""
    nparr = np.frombuffer(image_bytes, np.uint8)
    img   = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
    if img is None:
        raise ValueError("No se pudo decodificar la imagen.")

    # ✅ tempfile crea un archivo temporal compatible con cualquier OS
    ...
    try:
        ...
        if not ok:
            raise ValueError(f"No se pudo guardar la imagen temporal en: {tmp_path}")
